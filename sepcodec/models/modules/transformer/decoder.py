...
from torch.nn import LayerNorm, TransformerEncoder, TransformerEncoderLayer, TransformerDecoder, TransformerDecoderLayer
...
class Decoder(nn.Module):
    ...
    def forward(self, codes, original_embeddings = None, mix_embeddings = None, conditioning = None, padding_mask=None, use_embeddings = True, embeddings = None):
        ...
        decoded = self.transformer( tgt = input_, memory = mix_embeddings, tgt_key_padding_mask = padding_mask, memory_key_padding_mask = padding_mask)
        # shape B,T+1,d_model

        if self.first_run:
            print("shape coming out of decoder: ============")
            print(decoded.shape)
            
        # output_ = self.decoder_proj(decoded) ## to not share the output latent space with the decoder input space
